...
def get_args():
    ...
    parser.add_argument("reconstruction_window", help="Reconstruction window used to generate file: int or path to config file", type=str)
...
if __name__ == '__main__':
    ...
    voltage = re.search('(\d+(?:\.\d+)?) ?v', file_name, re.IGNORECASE)
    voltage = voltage.group() + " " if voltage else ""

    waveform_type = re.search('(burst|sine|square|triangle|noise)', file_name, re.IGNORECASE)
    ...
    degrees = re.search("[0-9]{1,} ?deg", file_name, re.IGNORECASE)
    if degrees:
